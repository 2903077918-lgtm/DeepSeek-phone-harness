...
// 用 mock transport + mock executor 验证：明文任务流转、E2EE 加解密、风险分级+确认、取消。
// 不连真实网络。用法：node test-cloud-service.mjs
import { createCloudService } from './src/cloud-service.mjs';
import { generateKeyPair, deriveSessionKey, encrypt, generateKeySalt } from './src/e2ee.js';
...
// ---- 测试1：明文 TASK_SUBMIT → executor → sendResult ----
{
  ...
  const svc = createCloudService({ transport: t, executor: ex, confirmPolicy: 'never', log: { info() {}, warn() {}, error() {} } });
  ...
  check('发送 ack', t.sent.some((s) => s.type === MSG_TYPES.TASK_ACK));
}

// ---- 测试2：E2EE 密文任务 → 解密后给 executor ----
{
  const agent = await generateKeyPair();
  const phone = await generateKeyPair();
  const salt = generateKeySalt();
  // Agent 侧从密文解密需要 deriveSessionKey(device.priv, phone.pub) = 手机 encrypt 的共享密钥
  const svc = createCloudService({
    transport: mockTransport(), executor: mockExecutor(),
    config: { cloud: { e2ee: { privateKey: agent.privateKey, peerPublicKey: phone.publicKey, salt } } },
    confirmPolicy: 'never', log: { info() {}, warn() {}, error() {} },
  });
  // 手机加密 prompt
  const phoneKey = await deriveSessionKey(phone.privateKey, agent.publicKey, salt);
  const cipher = encrypt(phoneKey, '删除临时文件', 'tsk_2');
  const t2 = mockTransport(); const ex2 = mockExecutor();
  const svc2 = createCloudService({
    transport: t2, executor: ex2,
    config: { cloud: { e2ee: { privateKey: agent.privateKey, peerPublicKey: phone.publicKey, salt } } },
    confirmPolicy: 'never', log: { info() {}, warn() {}, error() {} },
  });
  t2.recv({ v: 1, type: MSG_TYPES.TASK_SUBMIT, msgId: 'm2', seq: 2, payload: { taskId: 'tsk_2', promptCipher: cipher, requireConfirm: false } });
  await new Promise((r) => setTimeout(r, 20));
  check('E2EE 密文解密后给 executor', ex2.calls.length === 1 && ex2.calls[0].prompt === '删除临时文件');
  // result 有加密标记
  const res = t2.sent.find((s) => s.type === MSG_TYPES.TASK_RESULT);
  check('E2EE 结果加密', res && (res.payload.e2ee === true || res.payload.result_cipher !== undefined));
}

// ---- 测试3：high 风险 + confirmPolicy=high → 发 confirm.request，allow 后执行 ----
{
  const t = mockTransport(); const ex = mockExecutor();
  const svc = createCloudService({ transport: t, executor: ex, confirmPolicy: 'high', log: { info() {}, warn() {}, error() {} } });
